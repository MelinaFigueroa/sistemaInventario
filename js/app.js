...
async function actualizarDashboard() {
  ...
  const { data: posData } = await _supabase
    ...
    .select("cantidad");
  const total = posData
    ? posData.reduce((acc, curr) => acc + (curr.cantidad || 0), 0)
    : 0;
...
async function procesarPicking(pedidoId) {
  ...
  try {
    ...
    const { data: pedidoInfo, error: errP } = await _supabase
      .from("pedidos")
      .select(
        `cliente_nombre, pedido_detalle(cantidad, producto_id, productos(nombre, precios(precio_venta)))`,
      )
      .eq("id", pedidoId)
      .single();
    ...
    const { data: afipData, error: afipError } =
      await _supabase.functions.invoke("afip-invoice", {
        body: {
          pedidoId: pedidoId,
        ...
        headers: {
          apikey:
            "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9teWxydWNrcXZlc2VtcmxvbWVkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzAxNDE0MjIsImV4cCI6MjA4NTcxNzQyMn0.uWe09wGzCnYtIXPXTfhE7Z59iNda2YHjcqFBtKmcopU",
          Authorization:
            "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9teWxydWNrcXZlc2VtcmxvbWVkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzAxNDE0MjIsImV4cCI6MjA4NTcxNzQyMn0.uWe09wGzCnYtIXPXTfhE7Z59iNda2YHjcqFBtKmcopU",
        },
      });
    ...
    if (afipError || !afipData || !afipData.success) {
      ...
      throw new Error(
        afipData?.error || "La función de AFIP no respondió correctamente",
      );
    }
